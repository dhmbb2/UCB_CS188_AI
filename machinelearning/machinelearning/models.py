...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        relu1 = nn.ReLU(bias1)
        output = nn.AddBias(nn.Linear(relu1, self.w2), self.b2)
        return output
    ...
    def train(self, dataset):
        ...
        while loss >= 0.01:
            for x, y in dataset.iterate_once(self.batch_size):
                ...
                gradients = nn.gradients(lossnode, [self.w1, self.b1, self.w2, self.b2])
                for i in range(3):
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.wx = nn.Parameter(47, self.wordvec_size)
        self.bx = nn.Parameter(1, self.wordvec_size)
        self.wh = nn.Parameter(self.wordvec_size, self.wordvec_size)
        self.w1 = nn.Parameter(self.wordvec_size, self.hidden_size)
        self.b1 = nn.Parameter(1, self.hidden_size)
        self.w2 = nn.Parameter(self.hidden_size, 5)
        self.b2 = nn.Parameter(1, 5)
        self.parameters = [self.wx, self.bx, self.wh, self.w1, self.b1, self.w2, self.b2]
    ...
    def run(self, xs):
        ...
        for i in range(L):
            if i == 0:
                wordvec = nn.ReLU(nn.AddBias(nn.Linear(xs[i], self.wx), self.bx))
            else:
                wordvec = nn.ReLU(nn.AddBias(nn.Add(nn.Linear(xs[i], self.wx), nn.Linear(wordvec, self.wh)), self.bx))
        linear1 = nn.Linear(wordvec, self.w1)
        bias1 = nn.AddBias(linear1, self.b1)
        relu1 = nn.ReLU(bias1)
        output = nn.AddBias(nn.Linear(relu1, self.w2), self.b2)
        return output
    ...
    def train(self, dataset):
        ...
        while accuracy < 0.85:
            ...
            for xs, y in dataset.iterate_once(self.batch_size):
                lossnode = self.get_loss(xs, y)
                gradients = nn.gradients(lossnode, self.parameters)
